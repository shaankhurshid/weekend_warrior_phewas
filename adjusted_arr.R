...
sig_either <- fread(file='sig_either_150.csv')
...
ww[,activity_group := factor(activity_group,levels=c('Active - Regular','Active - WW','Inactive'))]
setkey(ww,sample_id)
...
for (i in list){
  ...
  # Fit cox model
  analysis_set[,activity_group := factor(activity_group,levels=c("Inactive","Active - Regular","Active - WW"))]
  ...
  weights <- data.frame(activity_group = levels(analysis_set$activity_group),
...
# Save out
write.csv(outer,file='arr_150.csv',row.names=F)
