...
# Set levels
ww[,activity_group := factor(activity_group,levels=c('Inactive','Active - WW','Active - Regular'))]
...
for (i in list){
  ...
  if (n_events < 10){
    ...
    n <- n+1; next}
  # Fit cox model
  model <- coxph(Surv(time_to_event,has_disease) ~ activity_group + accel_age + sex + race +
...
# Save out
write.csv(out,file='cox_ww_ref_inactive_150.csv',row.names=F)