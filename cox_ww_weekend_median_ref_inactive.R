...
# Set levels
ww[,weekend_activity_group_median := factor(weekend_activity_group_median,levels=c('Inactive','Active - WW','Active - Regular'))]
...
for (i in list){
  ...
  if (n_events < 10){
    ...
    hr_regular <- NA; lower_regular <- NA; upper_regular <- NA; z_regular <- NA; p_regular <- NA
    result <- data.table(disease,n_events,fu_median,fu_q1,fu_q3,
                         hr_ww,lower_ww,upper_ww,z_ww,p_ww,
                         hr_regular,lower_regular,upper_regular,z_regular,p_regular)
    ...
    n <- n+1; next}
  # Fit cox model
  model <- coxph(Surv(time_to_event,has_disease) ~ weekend_activity_group_median + accel_age + sex + race +
  ...
  hr_regular <- summary(model)$coefficients[2,2]; lower_regular <- summary(model)$conf.int[2,3]
  upper_regular <- summary(model)$conf.int[2,4]; z_regular <- summary(model)$coefficients[2,4]
  p_regular <- 2*pnorm(abs(z_regular),lower.tail=FALSE)
...
# Save out
write.csv(out,file='cox_ww_weekend_ref_inactive.csv',row.names=F)