...
results <- fread(file='v1_cox_ww_ref_inactive_150.csv')
...
# Remove NAs
results <- results[!is.na(hr_regular)]

# Fix p-values
results[,p_regular := 2*pnorm(abs(z_regular),lower.tail = FALSE)]

# FDR
fdr_out <- fdrtool(x=results$p_regular,statistic='pvalue')
...
for (i in 1:nrow(results)){
  ev <- evalues.HR(est=results$hr_regular[i],lo=results$lower_regular[i],hi=results$upper_regular[i],rare=results$rare_dz[i])
...
# shape
results[,shape := ifelse(hr_regular < 1,6,2)]
...
# Graphical y
results[,p_graphical := ifelse(p_regular < 1*10^-20,1*10^-20,p_regular)]

# Plot p-values
pdf(file='regular_150_ref_inactive.pdf',height=5,width=12,pointsize=5)
...
segments(0,-log10(max(results[sig==1]$p_regular)),nrow(results),-log10(max(results[sig==1]$p_regular)),col='#bd0026',lty=5)
...
pdf(file='regular_150_ref_inactive_effect.pdf',height=7,width=10,pointsize=5)
par(mar=c(32,3,1,8),oma=c(1,1,1,1))

plot(x=1:nrow(sig_results),y=sig_results$hr_regular,col=sig_results$col,
...
segments(1:nrow(sig_results),sig_results$lower_regular,1:nrow(sig_results),sig_results$upper_regular,col=sig_results$col,
         lwd=2)
...
write.csv(results,'regular_150_ref_inactive.csv',row.names=F)

# Noodles
sig_good <- sig_results[hr_regular < 1]
sig_bad <- sig_results[hr_regular > 1]
