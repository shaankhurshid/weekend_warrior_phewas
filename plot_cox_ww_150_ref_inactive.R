...
# Plot p-values
pdf(file='~/Documents/MGH Research/ww_phewas/plots/ww_150_ref_inactive.pdf',height=5,width=12,pointsize=5)
...
pdf(file='ww_150_ref_inactive_effect.pdf',height=7,width=10,pointsize=5)
...
write.csv(results,'ww_150_ref_inactive.csv',row.names=F)

# Noodles
sig_good <- sig_results[hr_ww < 1]
sig_bad <- sig_results[hr_ww > 1]
write.csv(sig_bad,file='ww_150_ref_inactive_bad.csv',row.names=F)
